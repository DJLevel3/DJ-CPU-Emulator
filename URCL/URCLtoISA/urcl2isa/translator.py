...
class Translator():
    ...
    def substitute(self, ins: "Instruction", clean=True):
        ...
        for l,line in enumerate(body):
            for i in range(len(ins.operands)):
                if (clean == True): body[l] = body[l].replace(f"@{chr(65+i)}", ins.operands[i].toStringClean())
                else: body[l] = body[l].replace(f"@{chr(65+i)}", ins.operands[i].toString())
        return body
