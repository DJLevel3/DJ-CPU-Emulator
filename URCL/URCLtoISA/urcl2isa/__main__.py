def main():
    ...
    def translateISA(program: Program, trans: Translator, clean=True):
        out: list[Block] = []
        for i,ins in enumerate(program.code):
            sub: list[str] = trans.substitute(ins,clean)
            if sub is None:
    ...
    start = timer()
    out = translateISA(main, translatorISA, True)
    end = timer()
